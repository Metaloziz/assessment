...
> Прототипная цепочка — механизм, через который объект получает свойства от другого объекта. Если собственного свойства нет, JavaScript ищет его в прототипе, затем выше по цепочке и останавливается на `null`.
>
> Так общие методы не копируются в каждый экземпляр: например, все массивы используют методы из `Array.prototype`. Это экономит память и позволяет объектам делить поведение без классов.
>
> При чтении поиск идёт по цепочке, а запись обычно создаёт собственное свойство и скрывает одноимённое свойство прототипа. Для проверки только своих данных нужен `Object.hasOwn`, а `in` намеренно учитывает и унаследованные свойства.
