...
> `class` — синтаксис для создания конструктора и прототипа, а не отдельная объектная модель. При `new` экземпляр получает ссылку `[[Prototype]]` на `Class.prototype`, где лежат общие методы.
>
> Это позволяет тысячам однотипных объектов использовать одну функцию метода вместо копии на каждом объекте. Статические члены принадлежат самому классу и подходят для фабрик или общих операций.
>
> `extends` связывает прототип потомка с прототипом родителя, а также сам конструктор потомка с родителем. В конструкторе наследника `this` нельзя использовать до `super()`. У класса есть дополнительная семантика: strict mode, запрет вызова без `new`, private names и правила `super`, поэтому он не сводится к текстовой замене на функцию-конструктор.
