...
> Прототипное наследование — это не копирование свойств родителя, а делегирование. Если экземпляр не находит метод у себя, JavaScript продолжает поиск в его прототипе и выше по цепочке.
>
> Так несколько объектов используют общее поведение без дублирования методов. `class extends` делает намерение заметнее, но под ним работает тот же прототипный механизм, что у `Object.create` и функций-конструкторов.
>
> Данные конкретного экземпляра записывают на него самого, а общие методы — в прототип. При наследовании конструкторов связывают `Child.prototype` с `Parent.prototype` и отдельно вызывают родительский конструктор для инициализации полей. Изменяемые массивы и объекты нельзя класть в общий прототип: их тогда разделят все экземпляры.
