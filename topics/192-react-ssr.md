...
- Не панацея: закрытые SPA без SEO часто лучше на CSR; SSR жрёт CPU сервера и не убирает ожидание JS для кликов.
...
SSR не «бесплатный upgrade» над CSR: HTML в первом ответе стоит CPU на сервере, сложнее деплой и выше риск hydration mismatch.

**Когда выигрыш слабый.** Закрытый кабинет за логином, админка, внутренний инструмент — SEO почти не нужен, пользователь и так ждёт приложение. Сильно интерактивный UI (редактор, доска, чат) всё равно упирается в JS: до `hydrateRoot` клики мёртвые, TTI не становится «мгновенным».

**Цена сервера.** На каждый запрос (или на поток) Node рендерит дерево. Без кэша HTML пиковая нагрузка бьёт по TTFB и деньгам. Персональные страницы (корзина, профиль) плохо кэшируются на CDN — чаще остаётся «рендер на запрос» или гибрид с SSG/ISR для публичных кусков.

**Ограничения рантайма.** На сервере нет `window`, `document`, `localStorage`. Всё, что от них зависит в `render`, ломает SSR или даёт mismatch. Браузерные API — в `useEffect` или в данные, уже согласованные с сервером.

**Гидратация всё ещё нужна.** SSR не убирает клиентский бандл для интерактивных частей: пользователь видит текст раньше, но «оживление» ждёт JS. Большое дерево → долгая гидратация и нагрузка на main thread; отсюда streaming, selective hydration и интерес к RSC (меньше JS на статику).

**Инфра и сопровождение.** Нужен Node (или аналог) на краю/бэке, одинаковый код для server/client entry, аккуратный fetch данных на сервере. Ошибка в «серверном» пути бьёт по первому байту всей страницы, не только по виджету.

Практично: публичный контент, лендинги, витрины, шаринг ссылок — SSR (или SSG) оправдан. SPA за авторизацией без SEO-требований — часто достаточно CSR.
