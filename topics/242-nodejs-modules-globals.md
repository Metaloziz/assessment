...
**Основные встроенные модули (http, fs) и глобальные переменные окружения (global, process)**
...
> Из коробки Node умеет принять HTTP-запрос и прочитать файл с диска — без npm-пакетов. Для этого есть встроенные модули ядра: `node:http` поднимает сервер, `node:fs` работает с файлами. Подключают их через `import` или `require('node:…')` — это API рантайма, а не зависимость из `package.json`.
>
> Зачем: уже на голом Node можно отдать ответ по HTTP и прочитать конфиг или статику. Рядом всегда `process` и `global` (в ESM — `globalThis`): аргументы CLI, переменные окружения, PID, выход из процесса — без отдельного импорта.
>
...
- `http` и `fs` — встроенные модули ядра с префиксом `node:`, не строки в `package.json`.
...
В лабе те же кейсы: живой `fs` и `process.env` на Node API, завершённый HTTP-ответ; «без end» и загрязнение `global` — на симуляционном стенде.
