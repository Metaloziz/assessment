...
> Объект без прототипа — словарь, который не наследует ничего от `Object.prototype`. Его создают через `Object.create(null)`, поэтому ключи `toString`, `constructor` и `__proto__` становятся обычными данными.
>
> Это полезно при подсчёте или хранении произвольных строковых ключей, особенно пришедших извне: имя ключа не конфликтует с унаследованным методом. Для обычной модели данных такой объект обычно не нужен.
>
> У него нет `toString` и `hasOwnProperty`, поэтому наличие ключа проверяют через `Object.hasOwn(obj, key)`. Такой словарь не даёт гарантированного выигрыша в производительности. Если нужны ключи любого типа, явные `get` и `set` или размер коллекции, чаще понятнее выбрать `Map`.
