...
> Ловушка: ждать префиксов от Sass — их даёт Autoprefixer. Дублировать nesting в Sass и `postcss-nesting` без нужды. Ставить autoprefixer **до** nesting — префиксы попадут не туда. Держать второй список браузеров только в Autoprefixer, игнорируя общий Browserslist — CSS и JS разъедутся по целям.
...
- **Autoprefixer** + **Browserslist** — стандартная пара для вендорных префиксов.
...
- Без актуального `caniuse-lite` / Browserslist префиксы и даунlevel синтаксис «уезжают» от реальности.
...
| Плагин | Роль |
| --- | --- |
| **autoprefixer** | `-webkit-` / `-ms-` по Browserslist |
...
## Autoprefixer и Browserslist

Autoprefixer без своего `overrideBrowserslist` читает тот же конфиг, что Babel (`package.json` или `.browserslistrc`). Список проверяют `npx browserslist`. Соседняя тема **Browserslist** — про общий контракт целей; здесь важно, что **CSS-префиксы** завязаны на него же.
...
- Отдельный `targets` только в Babel, забытый Browserslist для CSS.
- Считать PostCSS «препроцессором с переменными `$`» — `$` это Sass, не PostCSS.

Соседние темы: **SCSS/PostCSS** — общий пайплайн с препроцессором; **Browserslist** — общий список целей для Autoprefixer и Babel.
