...
1. Механизм темы **или** кластер с переключателем (на экране всё равно одна картина).
2. Вид картины под механизм: **живой механизм** (реальный API/DOM, если эффект виден в браузере) **или** живые UI-образцы (look) **или** узлы/метафора (граф, стек, паттерн без DOM-эффекта). Горизонтальный поток `A → B → C` — только если тема про пайплайн запроса. Эталоны: `react-portals`, `react-error-boundaries`, `layout-design-system`, `algorithms-stack-hashmap`.
3. Картина: мини-стенд **или** 3–6 узлов / 2–3 сцены с контролами — см. скилл визуализаций.
...
Если эффект механизма **виден в браузере** тем же API (portal, overflow clip, error boundary, live request) — картина = **живой мини-стенд**, не схема из `LabNode`. Эталоны: `react-portals`, `react-error-boundaries`. Подробности — в скилле визуализаций (§ «Живой механизм»).
