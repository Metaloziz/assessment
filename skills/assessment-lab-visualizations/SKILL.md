...
Эталон: **Кеширование. Подключение баз данных к серверу, CRUD** — `lab → cache`, затем **hit ↙** / **miss ↘**, ветки сходятся в общий ответ; для линейной мутации (UPDATE → invalidate) — отдельный горизонтальный ряд, не тот же fork.
