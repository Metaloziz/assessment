...
Схема на вкладке **«Решение проблемы»** — норма, не опция: 3–6 узлов, подсвечен **один** текущий шаг. Кнопки меняют картинку; лог не заменяет схему.

Если тема чисто про синтаксис/конфиг без потока — схема не обязательна (крошечный before/after или контраст в UI).

Не копировать **объём** `algorithms-graphs-list` (несколько механизмов в одном стенде) — только язык токенов и панелей.
...
- Несколько механизмов в одном стенде «как в graphs-list»
