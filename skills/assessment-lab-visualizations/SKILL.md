...
Новые live-API лабы (CORS, JWT, SQLi, WS…) переиспользуют **тот же** язык токенов и панелей: `LabVizPanel` / `LabNode` / `labVizStyles` из `app/src/components/lab/LabViz.tsx`. Не копировать блок `.viz` в CSS топика.

## Визуальный язык

- Панель схемы: **`LabVizPanel`** (не копипастить `.viz` в топик)
- Узлы: **`LabNode`** или `labVizStyles.node*` при кастомной вёрстке / GSAP
...
- Ошибка / риск: `--danger` / `--log-err`, без кричащих бейджей
